...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>{{ user.user_id or "&mdash;" }}</td>
      <td>
        {% for g in user.added %}
          <span class="badge badge-add">+ {{ g }}</span>
        {% endfor %}
        {% for g in user.removed %}
          <span class="badge badge-remove">&minus; {{ g }}</span>
        {% endfor %}
      </td>
...
def _ensure_business_category(
    ...
    """Add businessCategory to a group that lacks it."""
    safe_cn = escape_filter_chars(group_cn)
    conn.search(
        search_base=groups_ou,
        search_filter=f"(cn={safe_cn})",
        attributes=["businessCategory"],
    )
    if not conn.entries:
        return
    already_set = bool(conn.entries[0].entry_attributes_as_dict.get("businessCategory"))
    if not already_set:
        logging.info("  ~  %-24s  adding businessCategory=%s", group_cn, mtype)
        ldap_modify(conn, group_dn, {"businessCategory": [(MODIFY_ADD, [mtype])]}, dry_run)
...
def sync_group(
    ...
    _ensure_business_category(ldap_conn, group_dn, group_cn, groups_ou, mtype, dry_run)
    ...
    for member_dn in sorted(current_lower - desired_lower):
        ...
        changes.append({
            ...
            "user_exists": True, "first_name": "", "last_name": "", "user_id": "",
        })
